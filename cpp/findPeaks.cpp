	...
	for (int i=1;i<argc;i++) {
		...
		if (argv[i][0] == '-') {
			if (strcmp(argv[i],"-i")==0) {
				...
				gtfFile = argv[++i];
			} else if (strcmp(argv[i],"-inputPeaks")==0) {
				pf->inputPeaksFile = argv[++i];
			} else if (strcmp(argv[i],"-excludePeaks")==0) {
				pf->excludePeaksFile = argv[++i];
				...
				} else if (strcmp(argv[i],"superhistone")==0) {
					pf->stitchMode = REGION_MODE_HISTONE;
					pf->regionFlag = 1;
					pf->setPeakSize(500);
					pf->minDist = 1000;
...
void printCMD() {
	...
	fprintf(stderr, "\t\t\tsuperhistone (Super Enhancers from H3K27ac data, superEnhancers.txt)\n"); 
	fprintf(stderr, "\t\t\tmC (Cytosine methylation (BS-seq/methylC-seq), regions.txt)\n"); 
	fprintf(stderr, "\t\t\tdamid (DamID enrichment from DpnI digestion, regions.txt)\n"); 
	fprintf(stderr, "\t\t\tclip (CLIP-Seq enrichment, strand specific, peaks.txt)\n"); 
	...
	fprintf(stderr, "\t\t-inputPeaks <filename> (initial peaks to use for super enhancer merging/scoring)\n"); 
	fprintf(stderr, "\t\t-excludePeaks <filename> (regions to exclude from analysis, i.e. TSS regions for H3K27ac)\n"); 
